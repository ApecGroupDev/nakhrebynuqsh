    ...
    <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-foreground/5">
      <nav className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-3 items-center h-16 lg:h-20">
            ...
            <div className="hidden lg:flex items-center gap-8">
              {links.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="text-xs tracking-[0.2em] uppercase text-foreground/70 hover:text-gold transition-colors duration-200"
                >
                  {link.label}
                </Link>
              ))}
          ...
          >
            <span className="font-serif text-xl lg:text-2xl tracking-[0.15em]">
              NAKHRE
            </span>
            <span className="text-[9px] tracking-[0.3em] uppercase text-muted -mt-0.5">
              by Nuqsh
            </span>
          </Link>
